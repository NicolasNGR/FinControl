...
var categorias = [
  {id:1,nome:'Alimentação',icon:{id:'cart',p:'M7 18c-1.1 0-1.99.9-1.99 2S5.9 22 7 22s2-.9 2-2-.9-2-2-2zM1 2v2h2l3.6 7.59-1.35 2.45c-.16.28-.25.61-.25.96C5 16.1 6.9 18 9 18h12v-2H9.42c-.14 0-.25-.11-.25-.25l.03-.12.9-1.63H19c.75 0 1.41-.41 1.75-1.03l3.58-6.49A1 1 0 0023.46 5H5.21l-.94-2H1zm16 16c-1.1 0-1.99.9-1.99 2S15.9 22 17 22s2-.9 2-2-.9-2-2-2z',label:'Mercado'},cor:'#2D9A57'},
  {id:2,nome:'Transporte',icon:{id:'car',p:'M18.92 6.01C18.72 5.42 18.16 5 17.5 5h-11c-.66 0-1.21.42-1.42 1.01L3 12v8c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-1h12v1c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-8l-2.08-5.99zM6.5 16c-.83 0-1.5-.67-1.5-1.5S5.67 13 6.5 13s1.5.67 1.5 1.5S7.33 16 6.5 16zm11 0c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5zM5 11l1.5-4.5h11L19 11H5z',label:'Carro'},cor:'#1D4ED8'},
...
var lType='receita', selIcon=null, selColor='#2D9A57';
var fType='todos', fMonth='2025-05', fQ='';
...
function doLogin(){
  ...
  renderAll();
  loadSet('finance',document.querySelector('.ist'));
}
function logout(){document.getElementById('appWrap').style.display='none';document.getElementById('authWrap').style.display='flex'}
...
function fMonth(v){fMonth=v;renderLanc()}
...
function renderLanc(){
  var data=lancamentos.filter(function(l){
    ...
    if(fMonth&&!l.data.startsWith(fMonth))return false;
...
function delLanc(id){lancamentos=lancamentos.filter(function(l){return l.id!==id});renderLanc();toast('Lançamento excluído.','');}
...
function saveCat(){
  ...
  categorias.push({id:Date.now(),nome:nome,icon:selIcon,cor:selColor});
  renderCat();closeModal('mCat');document.getElementById('cNome').value='';toast('Categoria "'+nome+'" criada!','ok');
}
function delCat(id){categorias=categorias.filter(function(c){return c.id!==id});renderCat();toast('Categoria excluída.','');}
...
function pickIcon(el,id,p,label){
  ...
  document.getElementById('ipIcoPreview').innerHTML=icoSvg(p,20,selColor);
...
function saveMeta(){
  ...
  renderMetas();closeModal('mMeta');
  ['mNome','mTotal','mAtual','mData','mDesc'].forEach(function(id){document.getElementById(id).value=''});
  toast('Meta criada!','ok');
}
function delMeta(id){metas=metas.filter(function(m){return m.id!==id});renderMetas();toast('Meta excluída.','')}
